import {fetchJson,fetchSnapshot,normalize,type Coin} from '@/lib/market';
export async function GET(request:Request){
 ...
 const warnings:string[]=[];let pairs:any[]=[];let boosted=new Set<string>();let retrievedAt:string|null=null;
 const readPools=async(source:string)=>{const snapshot=await fetchSnapshot(source);retrievedAt=snapshot.retrievedAt;return snapshot.value;};
 try {
  if(addresses.length){pairs=await readPools('https://api.dexscreener.com/tokens/v1/solana/'+addresses.join(','));}
  else if(query){const r=await readPools('https://api.dexscreener.com/latest/dex/search?q='+encodeURIComponent(query));pairs=r.pairs||[];}
  else {
   ...
   pairs=await readPools('https://api.dexscreener.com/tokens/v1/solana/'+addrs.join(','));
  }
  ...
  return Response.json({coins:[...coins.values()].sort((a,b)=>b.score-a.score),asOf:retrievedAt,providerTimestampKnown:false,source:'DEX Screener',warnings,coverage:'Latest profiles and promoted tokens; up to 30 Solana tokens. Highest-liquidity returned pool per token. Not a whole-market ranking.',cacheSeconds:60},{headers:{'Cache-Control':'no-store'}});
