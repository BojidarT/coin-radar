...
export async function GET(){try{
 ...
 const r=await fetch('https://www.coindesk.com/arc/outboundfeeds/rss/',{signal:AbortSignal.timeout(12000)});if(!r.ok)throw Error();
 ...
 if(!items.length)throw Error();cached={at:Date.now(),items};return Response.json({items,asOf:new Date(cached.at).toISOString()});
 }catch{return Response.json({items:[],error:'News feed is temporarily unavailable.'},{status:502});}}
