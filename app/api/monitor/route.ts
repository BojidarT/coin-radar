...
export async function POST(request:Request){const user=await getChatGPTUser();if(!user)return Response.json({error:'Sign in required.'},{status:401});if(!sameOrigin(request))return Response.json({error:'Same-origin request required.'},{status:403});let lock:string|null=null;const lockId='monitor:'+user.userId;
 ...
 try{const r=await fetchJson('https://api.dexscreener.com/latest/dex/pairs/solana/'+[...new Set(positions.map(p=>p.pair))].join(','),10000);pools=r.pairs||[];}catch{providerAvailable=false;}
 ...
 }catch{return Response.json({error:'Position monitoring failed. Check positions directly in Phantom.'},{status:503});}finally{if(lock)await releaseLock(lockId,lock).catch(()=>{});}}
